#!/usr/bin/env python3
"""
Google Slides Skill - Environment Setup and Validation
...
def info(message):
    ...
    print(f"[extraslide-skill] {message}", file=sys.stderr)


def check_python_version():
    """Ensure Python 3.9+ is available."""
    if sys.version_info < (3, 9):  # noqa: UP036 - runs on user machines with older Python
        error(
            f"Python 3.9+ required, found {sys.version_info.major}.{sys.version_info.minor}",
            ["Install Python 3.9 or later from https://www.python.org/downloads/"],
        )
...
def install_dependencies():
    ...
    # Check if extraslide is already installed
    check_cmd = [str(VENV_PYTHON), "-c", "import extraslide; print('ok')"]
...
def main():
    ...
    output = {
        ...
        "message": "Environment ready. Use verify_access.py to test presentation access.",
    }
