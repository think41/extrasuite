...
@pytest.mark.asyncio
async def test_pull_writes_project_json(client: ScriptsClient, tmp_path: Path) -> None:
...
@pytest.mark.asyncio
async def test_pull_writes_gs_files(client: ScriptsClient, tmp_path: Path) -> None:
...
@pytest.mark.asyncio
async def test_pull_writes_html_files(client: ScriptsClient, tmp_path: Path) -> None:
...
@pytest.mark.asyncio
async def test_pull_writes_manifest(client: ScriptsClient, tmp_path: Path) -> None:
...
@pytest.mark.asyncio
async def test_pull_creates_pristine(client: ScriptsClient, tmp_path: Path) -> None:
...
@pytest.mark.asyncio
async def test_pull_saves_raw(client: ScriptsClient, tmp_path: Path) -> None:
    """Pull should save raw API responses when save_raw=True."""
    await client.pull("test_project", tmp_path, save_raw=True)
...
@pytest.mark.asyncio
async def test_pull_no_raw(client: ScriptsClient, tmp_path: Path) -> None:
    """Pull should skip raw files when save_raw=False."""
    await client.pull("test_project", tmp_path, save_raw=False)
    raw_dir = tmp_path / "test_project" / ".raw"
    assert not raw_dir.exists()


@pytest.mark.asyncio
async def test_pull_returns_file_list(client: ScriptsClient, tmp_path: Path) -> None:
    """Pull should return list of all written paths."""
    files = await client.pull("test_project", tmp_path)
