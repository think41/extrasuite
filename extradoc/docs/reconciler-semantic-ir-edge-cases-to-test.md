...
| Insert heading + paragraph + list + code block into an otherwise empty tab body | live `reconcile_v2` markdown workflow verification | Same-anchor mixed body inserts must lower as one coordinated sequence; range-based style and bullet requests cannot be emitted against an intermediate pre-shift anchor state. |
| Insert a paragraph before an unchanged TOC while deleting a later table in the same section | `reconcile_v2` anchor-aware body diff tests | Read-only body blocks are immutable anchors, not unsupported walls; editable spans on both sides must diff independently, and inserts before the anchor must preserve the raw body boundary for lowering. |
