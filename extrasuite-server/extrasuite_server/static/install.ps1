...
$dirs = @("$env:USERPROFILE\.claude\skills", "$env:USERPROFILE\.codex\skills")
foreach ($dir in $dirs) {
    New-Item -ItemType Directory -Force -Path $dir | Out-Null
    Copy-Item -Path "$tmp\extracted\gsheets" -Destination $dir -Recurse -Force
    Write-Host "  Installed to $dir\gsheets"
}
